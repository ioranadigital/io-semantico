-- ============================================================
-- MIGRATION 003: Force Clean (eliminar índices primero)
-- ============================================================

-- Eliminar índices explícitamente (en orden)
DROP INDEX IF EXISTS idx_optimizaciones_prioridad CASCADE;
DROP INDEX IF EXISTS idx_optimizaciones_estado CASCADE;
DROP INDEX IF EXISTS idx_optimizaciones_url CASCADE;
DROP INDEX IF EXISTS idx_optimizaciones_cliente CASCADE;

DROP INDEX IF EXISTS idx_asignaciones_estado CASCADE;
DROP INDEX IF EXISTS idx_asignaciones_keyword CASCADE;
DROP INDEX IF EXISTS idx_asignaciones_url CASCADE;

DROP INDEX IF EXISTS idx_urls_creado CASCADE;
DROP INDEX IF EXISTS idx_urls_tipologia CASCADE;
DROP INDEX IF EXISTS idx_urls_cliente CASCADE;

DROP INDEX IF EXISTS idx_palabras_clave_estado CASCADE;
DROP INDEX IF EXISTS idx_palabras_clave_nivel CASCADE;
DROP INDEX IF EXISTS idx_palabras_clave_cliente CASCADE;

DROP INDEX IF EXISTS idx_clientes_created_at CASCADE;
DROP INDEX IF EXISTS idx_clientes_nombre CASCADE;

-- Eliminar tablas (con CASCADE para las FK)
...
CREATE POLICY "Allow all reads" ON io_sem_clientes FOR SELECT USING (true);
CREATE POLICY "Allow all writes" ON io_sem_clientes FOR INSERT WITH CHECK (true);
CREATE POLICY "Allow all updates" ON io_sem_clientes FOR UPDATE USING (true) WITH CHECK (true);
CREATE POLICY "Allow all deletes" ON io_sem_clientes FOR DELETE USING (true);

-- TABLE 2: io_sem_palabras_clave
...
-- TABLE 3: io_sem_urls_rastreadas
...
-- TABLE 4: io_sem_asignaciones_kw_url
...
SELECT 'Schema reconstruido exitosamente ✅' as resultado;
